...
function displayProjects() {
	for (let i in Projects) {
		...
		let template = `
					<article class="project-card" data-context="${Projects[i].context}">
...
const displayFilters = function() {
	...
	listOfSelects.forEach(item => {
		...
		console.log(createDOMfilter(item, data[item])); 
		filtersBody += createDOMfilter(item, data[item]); 
	})

	console.log('<form id="filters-form">' + filtersBody + '</form>'); 
	...
	// 	let newInsert = `<option value="${year}">${year}</option>`; 
	// 	selectYearOptions += newInsert; 
	// })
	// let selectYear = `<div class="select-group">
	// 	<label for="year-select">Année</label>
	// 	<select name="years" id="year-select">
	// 		${selectYearOptions}
	// 	</select>
	// </div>`; 
	// document.querySelector('#filters-form').insertAdjacentHTML('beforeend', selectYear); 
