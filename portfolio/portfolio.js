...
function filterProjects(filter) {
	...
	return Projects.filter(project => {
		if (
			filter.year.indexOf(project.year) > -1 
			&& filter.context.indexOf(project.context) > -1
			&& arraysHaveCommonElements(project.skills, filter.skills)
			&& arraysHaveCommonElements(project.techno, filter.techno)
		) {
			return true; 
		}
		return false; 
