...
/**
 * Return an Array of Projects according to filters
 * @param { Array } year 
 * @param { Array } skills 
 * @param { Array } techno 
 * @param { Array } context 
 */
function filterProjects2(year, skills, techno, context) {
	if ( !Array.isArray(year) || !Array.isArray(year) || !Array.isArray(year) || !Array.isArray(year) ) {
		return new Error('non accepted filter element'); 
	}
